...
plot_pca <- function(pca)
  ...
  scale_fill_manual("", values = c("orange", "blue"), labels = c("Inne eukarioty", "Plasmodiidae")) +
...
p3 <- ggplot(cmp_dat, aes(x = MCC, y = MCC_p)) +
  ...
  my_theme +
  scale_x_continuous("MCC (inne eukarioty)") +
  scale_y_continuous("MCC (Plasmodiidae)") +
  scale_fill_manual("Długość alfabetu\n(peptyd sygnałowy)", values = c("dodgerblue", "firebrick1")) +
  scale_color_manual("Długość alfabetu\n(miejsce cięcia)", values = c("dodgerblue", "firebrick1")) +
  ggtitle("C")
